...
Token* Scanner::nextToken() {
    ...
    if (isdigit(c)) {
        ...
        if (word == "printf") {
            ...
            token = new Token(Token::MODIFIER_TYPE, word, 0, word.length());
        }
        ...
        switch(c) {
            ...
            case '{': token = new Token(Token::CHI, c); break;
            case '}': token = new Token(Token::CHD, c); break;
            case '#': token = new Token(Token::HT, c); break;
            case '=':
            ...
            case '\"':
                if (current+1<input.length() ) {
                    ...
                    token = new Token(Token::STRING, input.substr(first+1, current - (first+1)), 0, current - first);
                    break;
                }
